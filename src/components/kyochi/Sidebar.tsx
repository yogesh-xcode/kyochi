...
export function Sidebar({ navSections, role, mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 k-overlay z-30 lg:hidden transition-opacity ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
        onClick={onMobileClose}
        aria-hidden="true"
      />
      <aside
        className={`w-60 k-surface border-r k-border-soft flex flex-col fixed h-full z-40 transition-transform duration-300 ease-out shadow-sm ${mobileOpen ? "translate-x-0" : "-translate-x-full"
          } lg:translate-x-0`}
      >
        <div className="h-15.5 px-3.5 shrink-0 flex items-center gap-2 border-b k-border-soft">
              ...
              <div className="space-y-1">
                {section.items.map((item) => (
                  <Link
                    ...
                    href={item.href}
                    onClick={onMobileClose}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all text-[12px] font-semibold ${isItemActive(item.href)
                      ? "bg-[linear-gradient(90deg,rgba(212,175,53,0.18),rgba(212,175,53,0.04))] k-brand-strong border k-brand-border-soft"
                      : "k-text-body hover:bg-(--k-color-surface-muted) border border-transparent"
                      }`}
                  >
